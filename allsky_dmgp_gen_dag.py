...
with open(dag_dir + dag_name, 'w') as f:
    ...
    N = int(5.0e+06)
    for ephem in EPHs:
        datafile = os.path.join(datadir, 'nano11_{}_noDMX.pkl'.format(ephem))
        noisefile = os.path.join(datadir, 'nano11_setpars.pkl')

        # no BayesEphem
        outdir = os.path.join(dag_dir,"{0:s}_dmgp/all/".format(ephem))
        ...
        f.write('VARS {0:d} dmgp="--dmgp"\n'.format(job_ID))
        f.write('VARS {0:d} N="{1:d}"\n'.format(job_ID, N))
        f.write('\n')

        # with BayesEphem
        outdir = os.path.join(dag_dir,"{0:s}_dmgp_BE/all/".format(ephem))
        if not os.path.exists(outdir):
            os.makedirs(outdir)
        job_ID = random.getrandbits(128)
        f.write('JOB {0:d} {1:s}\n'.format(job_ID, subfile))
    ...
    for ephem in EPHs:
        ...
        for tmin,cent,tmax in tlim:
            outdir = os.path.join(dag_dir,"{0:s}_dmgp/{1:.2f}/".format(ephem, cent))
            if not os.path.exists(outdir):
                os.makedirs(outdir)
            job_ID = random.getrandbits(128)
            f.write('JOB {0:d} {1:s}\n'.format(job_ID, subfile))
        ...
        # with BayesEphem
        for tmin,cent,tmax in tlim:
            outdir = os.path.join(dag_dir,"{0:s}_dmgp_BE/{1:.2f}/".format(ephem, cent))
            if not os.path.exists(outdir):
                os.makedirs(outdir)
            job_ID = random.getrandbits(128)
            f.write('JOB {0:d} {1:s}\n'.format(job_ID, subfile))
